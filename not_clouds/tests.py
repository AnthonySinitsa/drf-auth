...
class CloudTests(TestCase):
  ...
  def setUpTestData(cls):
    ...
    test_thing = NotCloud.objects.create(name="rake", owner=testuser1, description="Better for collecting leaves than shovel.")
    test_thing.save()
    
    test_thing2 = NotCloud.objects.create(name="shovel", owner=testuser1, description="Better for digging than rake.")
    test_thing2.save()

  def test_things_model(self):
    thing = NotCloud.objects.get(id=1)
  ...
  def test_things_2(self):
    thing = NotCloud.objects.get(id=2)
    actual_owner = str(thing.owner)
    actual_name = str(thing.name)
    actual_description = str(thing.description)
    self.assertEqual(actual_owner, "testuser1")
